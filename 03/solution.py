...
def find_closest(seq):
  lines = seq.split()
  visited = set(crawl_wire(lines[0]))
  closest = min(abs(loc[0]) + abs(loc[1]) for loc in crawl_wire(lines[1]) if loc in visited)
  return closest


def find_closest2(seq):
  lines = seq.split()
  visited = set(crawl_wire(lines[0]))
  first = list(crawl_wire(lines[0]))
  b = 0
  results = []
  for b_value in crawl_wire(lines[1]):
    b = b + 1
    if b_value in visited:
      a = 0
      for a_value in first:
        a = a + 1
        if a_value == b_value:
          results.append(a+b)
          break
...
if __name__ == "__main__":
  print(f"Closest intersection {find_closest(my)}")
  print(f"First intersection {find_closest2(my)}")
